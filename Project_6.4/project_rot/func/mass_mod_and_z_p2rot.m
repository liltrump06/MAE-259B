function [imposeacc,M_modinv,nlist,interlist] = mass_mod_and_z_p2rot(q_new,M,conmap,velop)
...
for i =1:N
    if conmap(i,1) == 1
        if conmap(i,2) == 0
            ...
            nlist(i,:) = normvec;
            imposeacc(2*i-1:2*i) = Rc*([x0;y0] - [xpos;ypos]) / dt;
            %imposeacc(2*i-1:2*i) = Rc*([x0;y0] - [xpos;ypos]) / dt - normvec * dot(vpoint, normvec);
            S = Sformod(normvec);
