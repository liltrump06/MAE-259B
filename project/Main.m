...
l = 0.2; % length
r0 = 0.001; % radius of the rod
rou = 1000; % density kg/m3
...
mkref = zeros(N-2,1);
err = 100;
free_index = [1:4*round(N/2-4),4*round(N/2+4)+4:4*N-1];
...
for j = 1:(time/dt)% time loop
    ...
    while err >1e-6 % error loop
        ...
        [F,Jaco] = computeF_J(N,q_new,l_k,EA,m1,m2,GJ,EI,kappanew,mkref,g,M,e_dis(:,j));% compute F and J
